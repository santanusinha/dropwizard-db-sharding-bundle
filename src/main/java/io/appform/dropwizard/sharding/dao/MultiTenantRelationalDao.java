...
public class MultiTenantRelationalDao<T> implements ShardedDao<T> {
    ...
    private final class RelationalDaoPriv extends AbstractDAO<T> {
        ...
        protected T get(final Object lookupKey) {
        ...
        private QuerySpec<T, T> getDetachedCriteria(Object lookupKey) {
        ...
        T get(QuerySpec<T, T> criteria) {
            return uniqueResult(createQuery(currentSession(), entityClass, criteria).setLockMode(LockModeType.READ));
        }

        T getLocked(Object lookupKey, UnaryOperator<QuerySpec<T, T>> criteriaUpdater, LockMode lockMode) {
            return uniqueResult(
                    createQuery(currentSession(), entityClass, criteriaUpdater.apply(getDetachedCriteria(lookupKey)))
                            .setLockMode(lockMode.toJpaLockMode()));
        }
        ...
        List<T> run(final QuerySpec<T, T> criteria) {
    ...
    public Map<Integer, List> run(String tenantId, QuerySpec<T, T> criteria) {
        return run(tenantId, criteria, Function.<T, T>identity());
    }
     ...
     */
    @SuppressWarnings("rawtypes")
    public <U> U run(String tenantId, QuerySpec<T, U> criteria,
        ...
        val output = IntStream.range(0, daos.get(tenantId).size())
                ...
                .collect(Collectors.toMap(Function.identity(), shardId -> {
                    ...
                    OpContext<List> opContext = RunWithCriteria.<T>builder()

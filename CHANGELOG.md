...
- Removed the unused `shardingOptions` constructor parameter and `@Getter` from `MultiTenantLookupDao`, `MultiTenantRelationalDao`, `MultiTenantCacheableLookupDao`, and `MultiTenantCacheableRelationalDao`. The field was write-only (assigned but never read internally) and its getter had no callers anywhere in the codebase.

  **Reason**: This is a breaking change for any consumer that directly constructed DAOs, imported the old `io.appform.dropwizard.sharding.DBShardingBundleBase`/`MultiTenantDBShardingBundleBase`/`BundleCommonBase` package paths, called the two-arg `ShardCalculator` methods, relied on `ShardedDao<T>`/`dao.getShardCalculator()`, or passed/read `shardingOptions` on these DAO constructors/getters. It removes a static/global shard-calculator registry in favor of instances scoped to and owned by the bundle, eliminating a class of bugs where shard-routing state could leak or be shared unexpectedly across tenants.
